...
export const hookNSToken = (baseAddr: BaseAddr) => {
    ...
    {
        const funcName = 'night::NSToken::next(void)'
        const targetAddr = baseAddr.resolveAddress('0x00410C40')
        if (targetAddr != null) {
            let i = 0
            const arg: Record<string, NativePointer> = {}
            Interceptor.attach(targetAddr, { // Intercept calls to our SetAesDecrypt function
                ...
                */
                onEnter: function (args) {
                    try {
                        ++i
                        this.index = i
                        if (this.index != 396 && this.index != 397) return
                        console.log(`${funcName} - onEnter${this.index}`);
                        console.log('[+] Called targetAddr:' + targetAddr);
                        // console.log('[+] Ctx: ' + args[-1]);
                        // console.log('[+] FormatString: ' + Memory.readAnsiString(args[0])); // Plaintext
