...
export const hookRule = (baseAddr: BaseAddr) => {
    ...
    // {
    //     const funcName = 'WXSS::CSSTreeLib::MarkSelectorRule::MarkGood(WXSS::CSSTreeLib::CSSSyntaxTree *)'
    //     const targetAddr = baseAddr.resolveFunctionAddress(funcName)
    //     // ReadFile
    //     if (targetAddr != null) {
    //         let i = 0;
    ...
    {
        const funcName = 'WXSS::CSSTreeLib::RewriteSelectorRule::MarkGood(WXSS::CSSTreeLib::CSSSyntaxTree *)'
        const targetAddr = baseAddr.resolveFunctionAddress(funcName)
        // ReadFile
        if (targetAddr != null) {
            let i = 0;
                ...
                */
                onEnter: function (args) {
                    try {
                        ++i
                        this.index = i
                        this.output = this.index >= 75 && this.index <= 80
                        if (!this.output) return 
                        console.log(`${funcName} - onEnter${this.index}`);
                        console.log('[+] Called targetAddr:' + targetAddr);
                        // console.log('[+] Ctx: ' + args[-1]);
